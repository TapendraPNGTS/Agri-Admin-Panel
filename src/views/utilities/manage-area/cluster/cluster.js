...
import { updateAllCluster } from "../../../../redux/redux-slice/cluster.slice";
...
export default function DataTable() {
  ...
  const getAllCluster = useCallback(async () => {
    try {
      const cluster = await clusterApi.getAllCluster({});
      if (!cluster || !cluster.data.data) {
        return toast.error("no latest cluster available");
      } else {
        dispatch(updateAllCluster(cluster.data.data));
        return;
      }
    } catch (error) {
      console.error(error);
                            ...
                            <TableCell align="center" sx={{ pr: 3 }}>
                              ...
                              >
                                <Tooltip
                                  ...
                                  title="Edit"
                                  onClick={(e) => {
                                    navigate(`/edit-cluster/${row.ClusterLID}`);
                                  }}
                                  data-target={`#`}
                                >
                                  <IconButton
