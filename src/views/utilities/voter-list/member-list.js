...
import { updateAllMember } from "redux/redux-slice/member-list.slice";
...
export default function Users() {
  ...
  const rows = useSelector((state) => state.member.Voters);
  ...
  const getAllMember = useCallback(async () => {
    try {
      const member = await memberApi.getAllMember();
      if (!member || !member.data.data) {
        return toast.error("no latest member available");
      } else {
        dispatch(updateAllMember(member.data.data));
        return;
      }
    } catch (error) {
      console.error(error);
